...
export const ReqList = ({ fetchRequests }) => {
    const reduxRequests = useSelector((store) => store.requests);

    const [requests, setRequests] = useState(reduxRequests);
    ...
    const reviewReq = async (id, status) => {
        try {
            await axios.post(
                BaseUrl + `/connectionRequest/review/${id}/${status}`,
                {},
                ...
                {requests.map((connection) => (
                    <div
                        ...
                        className={`rounded-3xl border p-6 backdrop-blur-xl transition-all duration-300 ${
                            isDark
                                ? "bg-white/3 border-white/10 hover:border-sky-500/30 hover:bg-white/5"
                                : "bg-white border-black/5 hover:border-sky-300 shadow-md"
                        }`}
                    >
                        <div className="flex items-center gap-5">
                            <img
                        ...
                        <div className="flex items-center gap-3 mt-5">
                            <button
                                className="flex-1 bg-sky-500 hover:bg-sky-400 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors"
                                onClick={() =>
                                    reviewReq(connection._id, "accepted")
                                }
                            >
                                ✓ Accept
                            </button>
                            ...
                            <button
                                onClick={() =>
                                    reviewReq(connection._id, "rejected")
                                }
